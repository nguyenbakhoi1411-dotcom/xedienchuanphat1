...
import { ReceiptType } from "@/features/cash/types";
...
export default function NewReceiptPage() {
  ...
  const [lyDoNop, setLyDoNop] = useState("Thu tiền của");
  const [kemTheo, setKemTheo] = useState("");
  
  const [ngayHachToan, setNgayHachToan] = useState(() => new Date().toISOString().slice(0, 10));
  const [ngayPhieuThu, setNgayPhieuThu] = useState(() => new Date().toISOString().slice(0, 10));
  const [soPhieuThu, setSoPhieuThu] = useState("PT_AUTO");

  // Grid State
  const [lines, setLines] = useState<ReceiptLine[]>([
    {
      id: crypto.randomUUID(),
      dienGiai: "Thu tiền của",
      tkNo: "1111",
      tkCo: "",
      soTien: 0,
      nghiepVu: "",
  ...
  const handlePasteExcel = (e: ClipboardEvent<HTMLTableSectionElement>) => {
    ...
    rows.forEach((rowStr, i) => {
      ...
      const newLineObj: ReceiptLine = {
        id: crypto.randomUUID(),
        dienGiai: cols[0] || lyDoNop,
        tkNo: cols[1] || "1111",
        tkCo: cols[2] || "",
        soTien: parsedSoTien,
        nghiepVu: cols[4] || "",
  ...
  const handleSave = async (confirmNow: boolean) => {
    ...
    // Determine ReceiptType based on the first line's credit account
    let receiptType: ReceiptType = "OTHER";
    const mainCredit = lines[0]?.tkCo || "";
    if (mainCredit.startsWith("511")) receiptType = "SALE";
    else if (mainCredit.startsWith("131")) receiptType = "DEBT";

    // Combine lines description if multiple
    let combinedDesc = lyDoNop;
    if (lines.length > 1) {
    ...
    try {
      const payload = {
        receiptDate: ngayPhieuThu,
        receiptType,
        amount: tongTien,
        payerName: nguoiNop || tenDoiTuong,
        description: combinedDesc,
        confirmNow,
      };
      
      await cashApi.createReceipt(payload);
      toast.success(confirmNow ? "Đã xác nhận phiếu thu" : "Đã lưu nháp");
      router.push("/cash?tab=phieu-thu");
    } catch (err: any) {
      toast.error(err.response?.data?.message || "Lỗi lưu phiếu thu");
    } finally {
      setLoading(false);
    }
  };
    ...
    <div className="flex-1 flex flex-col min-h-full bg-[#f4f5f8]">
      ...
      <div className="bg-white border-b px-4 py-3 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          <Button variant="ghost" size="sm" onClick={() => router.push("/cash?tab=phieu-thu")}>
          ...
          <div className="flex items-center gap-2">
            ...
            <h1 className="text-xl font-semibold text-slate-800">Phiếu thu</h1>
          </div>
        </div>
        <div className="flex items-center gap-2">
          <Button variant="outline" onClick={() => router.push("/cash?tab=phieu-thu")}>Hủy</Button>
          <Button 
            disabled={loading} 
      ...
      <div className="flex-1 p-4 overflow-auto">
        <div className="bg-[#eaf4f4] rounded-t-lg border border-b-0 p-4 shadow-sm grid grid-cols-1 lg:grid-cols-12 gap-6 relative">
          ...
          <div className="lg:col-span-8 grid grid-cols-2 gap-x-6 gap-y-4">
            <div className="flex items-center gap-2">
              ...
              <input value={maDoiTuong} onChange={e => setMaDoiTuong(e.target.value)} className="flex-1 border border-slate-300 rounded px-2 py-1.5 text-sm outline-none focus:border-emerald-500" placeholder="Mã KH/NCC/NV..." />
            </div>
            <div className="flex items-center gap-2">
              <label className="w-28 text-sm font-medium text-slate-700">Tên đối tượng</label>
              <input value={tenDoiTuong} onChange={e => setTenDoiTuong(e.target.value)} className="flex-1 border border-slate-300 rounded px-2 py-1.5 text-sm outline-none focus:border-emerald-500" />
            </div>

            <div className="flex items-center gap-2">
              <label className="w-28 text-sm font-medium text-slate-700">Người nộp</label>
              <input value={nguoiNop} onChange={e => setNguoiNop(e.target.value)} className="flex-1 border border-slate-300 rounded px-2 py-1.5 text-sm outline-none focus:border-emerald-500" />
            </div>
            <div className="flex items-center gap-2">
              <label className="w-28 text-sm font-medium text-slate-700">Địa chỉ</label>
              <input value={diaChi} onChange={e => setDiaChi(e.target.value)} className="flex-1 border border-slate-300 rounded px-2 py-1.5 text-sm outline-none focus:border-emerald-500" />
            </div>

            <div className="flex items-center gap-2 col-span-2">
              <label className="w-28 text-sm font-medium text-slate-700">Lý do nộp</label>
              <input value={lyDoNop} onChange={e => setLyDoNop(e.target.value)} className="flex-1 border border-slate-300 rounded px-2 py-1.5 text-sm outline-none focus:border-emerald-500" />
            </div>

            <div className="flex items-center gap-2">
              <label className="w-28 text-sm font-medium text-slate-700">Nhân viên</label>
              <input value={nhanVien} onChange={e => setNhanVien(e.target.value)} className="flex-1 border border-slate-300 rounded px-2 py-1.5 text-sm outline-none focus:border-emerald-500" />
            </div>
            <div className="flex items-center gap-2 flex-1">
              <label className="w-28 text-sm font-medium text-slate-700">Kèm theo</label>
              <input type="number" value={kemTheo} onChange={e => setKemTheo(e.target.value)} className="w-20 border border-slate-300 rounded px-2 py-1.5 text-sm outline-none focus:border-emerald-500" placeholder="SL" />
              <span className="text-sm text-slate-600">chứng từ gốc</span>
            </div>
          </div>

          <div className="lg:col-span-4 border-l border-slate-300 pl-6 space-y-4">
            <div className="flex items-center justify-between">
              <label className="text-sm font-medium text-slate-700">Ngày hạch toán</label>
              <input type="date" value={ngayHachToan} onChange={e => setNgayHachToan(e.target.value)} className="border border-slate-300 rounded px-2 py-1.5 text-sm w-40 outline-none focus:border-emerald-500" />
            </div>
            <div className="flex items-center justify-between">
              <label className="text-sm font-medium text-slate-700">Ngày phiếu thu</label>
              <input type="date" value={ngayPhieuThu} onChange={e => setNgayPhieuThu(e.target.value)} className="border border-slate-300 rounded px-2 py-1.5 text-sm w-40 outline-none focus:border-emerald-500" />
            </div>
            <div className="flex items-center justify-between">
              <label className="text-sm font-medium text-slate-700">Số phiếu thu</label>
              <input type="text" disabled value={soPhieuThu} className="border border-slate-300 rounded px-2 py-1.5 text-sm w-40 bg-slate-100 outline-none focus:border-emerald-500" />
            </div>
            ...
            <div className="pt-4 text-right">
              ...
              <div className="text-xs text-emerald-600 font-medium mt-1 italic">
        ...
        <div className="bg-white rounded-b-lg border border-slate-300 shadow-sm">
          ...
          <div className="overflow-x-auto">
            <table className="w-full text-sm border-collapse min-w-[1000px]">
              <thead>
                <tr className="bg-[#a2d2c9] text-slate-800">
              ...
              <tbody onPaste={handlePasteExcel}>
                {lines.map((line, i) => (
                  <tr key={line.id} data-index={i} className="hover:bg-slate-50 transition-colors group">
                    ...
                    <td className="border border-slate-300 px-0 py-0">
                      <input 
                        ...
                        className="w-full h-full min-h-[32px] px-2 outline-none focus:ring-1 focus:ring-emerald-500 focus:bg-white bg-transparent" 
                      />
                    </td>
                    <td className="border border-slate-300 px-0 py-0">
                      <input 
                        ...
                        className="w-full h-full min-h-[32px] px-2 text-center outline-none focus:ring-1 focus:ring-emerald-500 focus:bg-white bg-transparent" 
                      />
                    </td>
                    <td className="border border-slate-300 px-0 py-0">
                      <input 
                    ...
                    <td className="border border-slate-300 px-0 py-0">
                      <input 
                        ...
                        className="w-full h-full min-h-[32px] px-2 text-right font-medium text-emerald-600 outline-none focus:ring-1 focus:ring-emerald-500 focus:bg-white bg-transparent" 
                      />
                    </td>
                    <td className="border border-slate-300 px-0 py-0">
                      <input 
