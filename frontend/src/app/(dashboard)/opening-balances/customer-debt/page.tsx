...
const MOCK_CUSTOMERS = [
  { code: "KH001", name: "Công ty Cổ phần Xây dựng Hòa Bình" },
  { code: "KH002", name: "Công ty TNHH TM DV Vạn Đạt" },
  { code: "KH003", name: "Tập đoàn Vingroup" },
  { code: "KH004", name: "Công ty Cổ phần Sữa Việt Nam (Vinamilk)" },
  { code: "KH005", name: "Nguyễn Văn A" }
];

export default function OBCustomerDebtPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await fetch("http://localhost:3001/api/opening-balances/customer-debt?namKeToan=2026");
  ...
  const openModal = () => {
    if (customers.length > 0) {
      setGridItems(customers.map((c, idx) => ({ ...c, id: idx.toString() })));
    } else {
      handleAddRow();
    }
    setIsModalOpen(true);
  ...
  const handleAddRow = () => {
    setGridItems(prev => [
      ...prev,
      {
        ...
        maKhachHang: "KH001",
        tenKhachHang: "Công ty Cổ phần Xây dựng Hòa Bình",
        soTienPhaiThu: 0,
        soTienKhachUng: 0
      }
    ]);
    ...
    try {
      const payload = {
        ...
        items: gridItems.map(item => ({
          ...
          soTienPhaiThu: item.soTienPhaiThu || 0,
          soTienKhachUng: item.soTienKhachUng || 0,
          customerId: Date.now() + Math.floor(Math.random() * 1000)
        }))
      };

      const res = await fetch("http://localhost:3001/api/opening-balances/customer-debt/batch", {
        method: "POST",
      ...
      if (res.ok) {
        alert("Lưu công nợ khách hàng thành công!");
        fetchData();
  ...
  const mainTotalDebit = customers.reduce((sum, item) => sum + Number(item.soTienPhaiThu || 0), 0);
  const mainTotalCredit = customers.reduce((sum, item) => sum + Number(item.soTienKhachUng || 0), 0);
  
  const gridTotalDebit = gridItems.reduce((sum, item) => sum + Number(item.soTienPhaiThu || 0), 0);
  const gridTotalCredit = gridItems.reduce((sum, item) => sum + Number(item.soTienKhachUng || 0), 0);
    ...
    <div className="bg-[#f4f7f6] min-h-[calc(100vh-4rem)] pb-32">
      ...
      <div className="px-6 py-4 flex flex-col gap-4 bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">Công nợ khách hàng</h1>
      ...
      <div className="p-6">
        <div className="bg-white rounded border border-slate-300 shadow-sm overflow-hidden">
          <table className="w-full text-sm text-left border-collapse select-none">
            <thead className="bg-[#bce6e4] text-[#006b66]">
              <tr>
                ...
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Mã khách hàng</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Tên khách hàng</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold text-right">Dư Nợ (Phải thu)</th>
                <th className="px-3 py-2 font-bold text-right">Dư Có (Khách ứng)</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                customers.map((item, idx) => (
                  <tr key={idx} className="border-b border-slate-200 hover:bg-emerald-50/30">
                    ...
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700 font-semibold">{item.maKhachHang}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700">{item.tenKhachHang}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-right font-semibold">{formatNum(Number(item.soTienPhaiThu))}</td>
                    <td className="px-3 py-2 text-right font-semibold">{formatNum(Number(item.soTienKhachUng))}</td>
                  </tr>
                ))
              )}
            </tbody>
          ...
          <div className="p-3 border-t border-slate-200 text-sm flex justify-between items-center text-slate-500">
            <div>Tổng số: <span className="font-bold text-slate-700">{customers.length}</span> bản ghi</div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-[#f4f7f6] flex flex-col h-screen animate-in fade-in">
          <div className="px-6 py-3 flex items-center justify-between border-b border-slate-200 bg-white shadow-sm">
            <h1 className="text-xl font-bold text-slate-800">Nhập công nợ khách hàng</h1>
              ...
              <div className="flex-1 overflow-y-auto relative">
                <table className="w-full text-sm text-left border-collapse select-none">
                  <thead className="bg-[#bce6e4] text-[#006b66] sticky top-0 z-10 border-b border-slate-300 shadow-sm">
                    <tr>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-48">Mã khách hàng</th>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9]">Tên khách hàng</th>
                      ...
                      <tr key={item.id} className="border-b border-slate-200 hover:bg-emerald-50/30">
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <GenericCombobox 
                            ...
                            onChange={(code) => handleCellChange(item.id, 'maKhachHang', code)} 
                            onNameChange={(name) => handleCellChange(item.id, 'tenKhachHang', name)}
                          />
                        </td>
                        <td className="px-4 py-2 border-r border-slate-200 text-slate-700">{item.tenKhachHang}</td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <NumberInput value={item.soTienPhaiThu} onChange={(val) => handleCellChange(item.id, 'soTienPhaiThu', val)} />
                        </td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <NumberInput value={item.soTienKhachUng} onChange={(val) => handleCellChange(item.id, 'soTienKhachUng', val)} />
                        </td>
