...
import { ChevronDown, Search, FileDown, HelpCircle, X, Trash2, Check, Filter } from "lucide-react";
import { NumberInput, GenericCombobox, TextInput } from "../components";

const MOCK_FIXED_ASSETS = [
  { code: "TS001", name: "Xe ô tô Toyota Camry 2.0G" },
  { code: "TS002", name: "Máy xúc lật Komatsu" },
  { code: "TS003", name: "Dây chuyền sản xuất gạch" },
  { code: "TS004", name: "Hệ thống điện năng lượng mặt trời" },
  { code: "TS005", name: "Tòa nhà văn phòng công ty" }
];

export default function OBFixedAssetsPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await fetch("http://localhost:3001/api/opening-balances/fixed-assets?namKeToan=2026");
  ...
  const openModal = () => {
    if (assets.length > 0) {
      setGridItems(assets.map((c, idx) => ({ ...c, id: idx.toString() })));
    } else {
      handleAddRow();
    }
    setIsModalOpen(true);
  ...
  const handleCellChange = (id: string, field: string, value: any) => {
    setGridItems(prev => prev.map(item => {
      if (item.id === id) {
        ...
        // Auto calculate
        if (field === 'nguyenGia' || field === 'haoMonLuyKe') {
           newItem.giaTriConLai = Math.max(0, (newItem.nguyenGia || 0) - (newItem.haoMonLuyKe || 0));
        }
        return newItem;
      }
      return item;
  ...
  const handleAddRow = () => {
    setGridItems(prev => [
      ...prev,
      {
        ...
        maTscd: "TS001",
        tenTscd: "Xe ô tô Toyota Camry 2.0G",
    ...
    try {
      const payload = {
        ...
        items: gridItems.map(item => ({
          maTscd: item.maTscd,
          tenTscd: item.tenTscd,
          phongBan: item.phongBan,
          nguyenGia: item.nguyenGia || 0,
          haoMonLuyKe: item.haoMonLuyKe || 0,
          giaTriConLai: item.giaTriConLai || 0,
          assetId: Date.now() + Math.floor(Math.random() * 1000)
        }))
      };

      const res = await fetch("http://localhost:3001/api/opening-balances/fixed-assets/batch", {
        method: "POST",
      ...
      if (res.ok) {
        alert("Lưu tài sản cố định thành công!");
        fetchData();
  ...
  const mainTotalNG = assets.reduce((sum, item) => sum + Number(item.nguyenGia || 0), 0);
  const mainTotalHM = assets.reduce((sum, item) => sum + Number(item.haoMonLuyKe || 0), 0);
  const mainTotalCL = assets.reduce((sum, item) => sum + Number(item.giaTriConLai || 0), 0);

  const gridTotalNG = gridItems.reduce((sum, item) => sum + Number(item.nguyenGia || 0), 0);
  const gridTotalHM = gridItems.reduce((sum, item) => sum + Number(item.haoMonLuyKe || 0), 0);
  const gridTotalCL = gridItems.reduce((sum, item) => sum + Number(item.giaTriConLai || 0), 0);
    ...
    <div className="bg-[#f4f7f6] min-h-[calc(100vh-4rem)] pb-32">
      ...
      <div className="px-6 py-4 flex flex-col gap-4 bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">Tài sản cố định</h1>
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="p-1.5 border border-slate-300 rounded hover:bg-slate-50"><Filter className="w-4 h-4 text-emerald-600" /></button>
            ...
            <button onClick={openModal} className="flex items-center gap-2 px-6 py-1.5 bg-[#008f89] text-white rounded font-semibold hover:bg-[#007a75] shadow-sm">
              Nhập TSCĐ | <ChevronDown className="w-4 h-4" />
            </button>
          ...
          <table className="w-full text-sm text-left border-collapse select-none">
            <thead className="bg-[#bce6e4] text-[#006b66]">
              <tr>
                ...
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Mã TSCĐ</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Tên TSCĐ</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Phòng ban</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold text-right">Nguyên giá</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold text-right">Hao mòn lũy kế</th>
            ...
            <tbody>
              {loading ? (
                ...
                <tr><td colSpan={7} className="text-center py-4 text-slate-500">Chưa có dữ liệu...</td></tr>
              ) : (
                assets.map((item, idx) => (
                  <tr key={idx} className="border-b border-slate-200 hover:bg-emerald-50/30">
                    ...
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700 font-semibold">{item.maTscd}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700">{item.tenTscd}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700">{item.phongBan}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-right font-semibold">{formatNum(Number(item.nguyenGia))}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-right font-semibold">{formatNum(Number(item.haoMonLuyKe))}</td>
                    <td className="px-3 py-2 text-right font-semibold">{formatNum(Number(item.giaTriConLai))}</td>
                  </tr>
                ))
              )}
            </tbody>
            <tfoot className="bg-[#f0f0f0]">
              <tr>
                <td colSpan={4} className="px-3 py-2 font-bold border-r border-slate-300 text-center uppercase">Tổng</td>
                <td className="px-3 py-2 font-bold text-right border-r border-slate-300 text-emerald-700">{formatNum(mainTotalNG)}</td>
                <td className="px-3 py-2 font-bold text-right border-r border-slate-300 text-emerald-700">{formatNum(mainTotalHM)}</td>
                <td className="px-3 py-2 font-bold text-right text-emerald-700">{formatNum(mainTotalCL)}</td>
              </tr>
            </tfoot>
          </table>
          <div className="p-3 border-t border-slate-200 text-sm flex justify-between items-center text-slate-500">
            <div>Tổng số: <span className="font-bold text-slate-700">{assets.length}</span> bản ghi</div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-[#f4f7f6] flex flex-col h-screen animate-in fade-in">
          <div className="px-6 py-3 flex items-center justify-between border-b border-slate-200 bg-white shadow-sm">
            <h1 className="text-xl font-bold text-slate-800">Nhập tài sản cố định</h1>
              ...
              <div className="flex-1 overflow-y-auto relative">
                <table className="w-full text-sm text-left border-collapse select-none">
                  <thead className="bg-[#bce6e4] text-[#006b66] sticky top-0 z-10 border-b border-slate-300 shadow-sm">
                    <tr>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-48">Mã TSCĐ</th>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-64">Tên TSCĐ</th>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-48">Phòng ban</th>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-48 text-right">Nguyên giá</th>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-48 text-right">Hao mòn lũy kế</th>
                    ...
                    {gridItems.map((item) => (
                      <tr key={item.id} className="border-b border-slate-200 hover:bg-emerald-50/30">
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <GenericCombobox 
                            value={item.maTscd} 
                            items={MOCK_FIXED_ASSETS}
                            codeLabel="Mã TSCĐ"
                            nameLabel="Tên TSCĐ"
                            onChange={(code) => handleCellChange(item.id, 'maTscd', code)} 
                            onNameChange={(name) => handleCellChange(item.id, 'tenTscd', name)}
                          />
                        </td>
                        <td className="px-4 py-2 border-r border-slate-200 text-slate-700">{item.tenTscd}</td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <TextInput value={item.phongBan} onChange={(val) => handleCellChange(item.id, 'phongBan', val)} placeholder="Phòng ban" />
                        </td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <NumberInput value={item.nguyenGia} onChange={(val) => handleCellChange(item.id, 'nguyenGia', val)} />
                        </td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <NumberInput value={item.haoMonLuyKe} onChange={(val) => handleCellChange(item.id, 'haoMonLuyKe', val)} />
                        </td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <NumberInput value={item.giaTriConLai} onChange={(val) => handleCellChange(item.id, 'giaTriConLai', val)} />
                        </td>
                  ...
                  <tfoot className="bg-[#f0f0f0] text-slate-800 sticky bottom-0 z-10 border-t border-slate-300 shadow-[0_-2px_4px_rgba(0,0,0,0.05)]">
                    <tr>
                      <td colSpan={3} className="px-4 py-2 font-bold text-center border-r border-slate-300 uppercase">Tổng</td>
                      <td className="px-4 py-2 font-bold text-right border-r border-slate-300 text-lg text-emerald-700">{formatNum(gridTotalNG)}</td>
                      <td className="px-4 py-2 font-bold text-right border-r border-slate-300 text-lg text-emerald-700">{formatNum(gridTotalHM)}</td>
                      <td className="px-4 py-2 font-bold text-right border-r border-slate-300 text-lg text-emerald-700">{formatNum(gridTotalCL)}</td>
                      <td></td>
                    </tr>
                  </tfoot>
                </table>
