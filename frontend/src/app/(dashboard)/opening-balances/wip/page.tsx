...
import { ChevronDown, Search, FileDown, HelpCircle, X, Trash2, LayoutTemplate } from "lucide-react";
import { NumberInput, GenericCombobox, TextInput } from "../components";

const MOCK_WIP = [
  { code: "CT001", name: "Công trình xây dựng nhà xưởng giai đoạn 1" },
  { code: "CT002", name: "Nâng cấp hệ thống phần mềm ERP" },
  { code: "CT003", name: "Dự án thiết kế bộ nhận diện thương hiệu" },
  { code: "CT004", name: "Công trình đường nội bộ KCN" },
  { code: "CT005", name: "Lắp đặt dây chuyền sản xuất" }
];

export default function OBWIPPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await fetch("http://localhost:3001/api/opening-balances/wip?namKeToan=2026");
  ...
  const openModal = () => {
    if (wips.length > 0) {
      setGridItems(wips.map((c, idx) => ({ ...c, id: idx.toString() })));
    } else {
      handleAddRow();
    }
    setIsModalOpen(true);
  ...
  const handleAddRow = () => {
    setGridItems(prev => [
      ...prev,
      {
        ...
        maDoiTuong: "CT001",
        tenDoiTuong: "Công trình xây dựng nhà xưởng giai đoạn 1",
        loaiDoiTuong: "Công trình",
        giaTriDangDang: 0
      }
    ]);
  };
    ...
    try {
      const payload = {
        ...
        items: gridItems.map(item => ({
          ...
          giaTriDangDang: item.giaTriDangDang || 0,
          wipId: Date.now() + Math.floor(Math.random() * 1000)
        }))
      };

      const res = await fetch("http://localhost:3001/api/opening-balances/wip/batch", {
        method: "POST",
      ...
      if (res.ok) {
        alert("Lưu chi phí dở dang thành công!");
        fetchData();
  ...
  const mainTotalValue = wips.reduce((sum, item) => sum + Number(item.giaTriDangDang || 0), 0);
  const gridTotalValue = gridItems.reduce((sum, item) => sum + Number(item.giaTriDangDang || 0), 0);
    ...
    <div className="bg-[#f4f7f6] min-h-[calc(100vh-4rem)] pb-32">
      ...
      <div className="px-6 py-4 flex flex-col gap-4 bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">Chi phí sản xuất, kinh doanh dở dang</h1>
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <button className="p-1.5 border border-slate-300 rounded hover:bg-slate-50"><LayoutTemplate className="w-4 h-4 text-emerald-600" /></button>
      ...
      <div className="p-6">
        <div className="bg-white rounded border border-slate-300 shadow-sm overflow-hidden">
          <table className="w-full text-sm text-left border-collapse select-none">
            <thead className="bg-[#bce6e4] text-[#006b66]">
              <tr>
                ...
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Mã đối tượng THCP</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Tên đối tượng THCP</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Loại đối tượng</th>
                <th className="px-3 py-2 font-bold text-right">Giá trị dở dang</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                wips.map((item, idx) => (
                  <tr key={idx} className="border-b border-slate-200 hover:bg-emerald-50/30">
                    ...
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700 font-semibold">{item.maDoiTuong}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700">{item.tenDoiTuong}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700">{item.loaiDoiTuong}</td>
                    <td className="px-3 py-2 text-right font-semibold">{formatNum(Number(item.giaTriDangDang))}</td>
                  </tr>
                ))
              )}
            </tbody>
          ...
          <div className="p-3 border-t border-slate-200 text-sm flex justify-between items-center text-slate-500">
            <div>Tổng số: <span className="font-bold text-slate-700">{wips.length}</span> bản ghi</div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-[#f4f7f6] flex flex-col h-screen animate-in fade-in">
          <div className="px-6 py-3 flex items-center justify-between border-b border-slate-200 bg-white shadow-sm">
            <h1 className="text-xl font-bold text-slate-800">Nhập chi phí sản xuất, kinh doanh dở dang</h1>
              ...
              <div className="flex-1 overflow-y-auto relative">
                <table className="w-full text-sm text-left border-collapse select-none">
                  <thead className="bg-[#bce6e4] text-[#006b66] sticky top-0 z-10 border-b border-slate-300 shadow-sm">
                    <tr>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-48">Mã đối tượng THCP</th>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-64">Tên đối tượng THCP</th>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-48">Loại đối tượng</th>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-48 text-right">Giá trị dở dang</th>
                      ...
                      <tr key={item.id} className="border-b border-slate-200 hover:bg-emerald-50/30">
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <GenericCombobox 
                            ...
                            items={MOCK_WIP}
                            codeLabel="Mã ĐT"
                            nameLabel="Tên đối tượng THCP"
                            onChange={(code) => handleCellChange(item.id, 'maDoiTuong', code)} 
                            onNameChange={(name) => handleCellChange(item.id, 'tenDoiTuong', name)}
                          />
                        </td>
                        <td className="px-4 py-2 border-r border-slate-200 text-slate-700">{item.tenDoiTuong}</td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <TextInput value={item.loaiDoiTuong} onChange={(val) => handleCellChange(item.id, 'loaiDoiTuong', val)} placeholder="Loại đối tượng" />
                        </td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <NumberInput value={item.giaTriDangDang} onChange={(val) => handleCellChange(item.id, 'giaTriDangDang', val)} />
                        </td>
