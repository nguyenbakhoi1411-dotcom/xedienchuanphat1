...
const MOCK_PREPAID = [
  { code: "CP001", name: "Chi phí thuê văn phòng 1 năm" },
  { code: "CP002", name: "Bảo hiểm cháy nổ nhà xưởng" },
  { code: "CP003", name: "Chi phí marketing quý 1" },
  { code: "CP004", name: "Bản quyền phần mềm 1 năm" },
  { code: "CP005", name: "Chi phí đào tạo nhân viên" }
];

export default function OBPrepaidPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await fetch("http://localhost:3001/api/opening-balances/prepaid?namKeToan=2026");
  ...
  const openModal = () => {
    if (prepaids.length > 0) {
      setGridItems(prepaids.map((c, idx) => ({ ...c, id: idx.toString() })));
    } else {
      handleAddRow();
    }
    setIsModalOpen(true);
  ...
  const handleCellChange = (id: string, field: string, value: any) => {
    setGridItems(prev => prev.map(item => {
      if (item.id === id) {
        ...
        // Auto calculate
        if (field === 'tongGiaTri' || field === 'giaTriDaPhanBo') {
           newItem.giaTriConLai = Math.max(0, (newItem.tongGiaTri || 0) - (newItem.giaTriDaPhanBo || 0));
        }
        return newItem;
      }
      return item;
  ...
  const handleAddRow = () => {
    setGridItems(prev => [
      ...prev,
      {
        ...
        maKhoanChiPhi: "CP001",
        tenKhoanChiPhi: "Chi phí thuê văn phòng 1 năm",
        tongGiaTri: 0,
        giaTriDaPhanBo: 0,
        giaTriConLai: 0
      }
    ...
    try {
      const payload = {
        ...
        items: gridItems.map(item => ({
          maKhoanChiPhi: item.maKhoanChiPhi,
          tenKhoanChiPhi: item.tenKhoanChiPhi,
          tongGiaTri: item.tongGiaTri || 0,
          giaTriDaPhanBo: item.giaTriDaPhanBo || 0,
          giaTriConLai: item.giaTriConLai || 0,
          prepaidId: Date.now() + Math.floor(Math.random() * 1000)
        }))
      };

      const res = await fetch("http://localhost:3001/api/opening-balances/prepaid/batch", {
        method: "POST",
      ...
      if (res.ok) {
        alert("Lưu chi phí trả trước thành công!");
        fetchData();
  ...
  const mainTotalGT = prepaids.reduce((sum, item) => sum + Number(item.tongGiaTri || 0), 0);
  const mainTotalPB = prepaids.reduce((sum, item) => sum + Number(item.giaTriDaPhanBo || 0), 0);
  const mainTotalCL = prepaids.reduce((sum, item) => sum + Number(item.giaTriConLai || 0), 0);

  const gridTotalGT = gridItems.reduce((sum, item) => sum + Number(item.tongGiaTri || 0), 0);
  const gridTotalPB = gridItems.reduce((sum, item) => sum + Number(item.giaTriDaPhanBo || 0), 0);
    ...
    <div className="bg-[#f4f7f6] min-h-[calc(100vh-4rem)] pb-32">
      ...
      <div className="px-6 py-4 flex flex-col gap-4 bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">Chi phí trả trước</h1>
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={openModal} className="flex items-center gap-2 px-6 py-1.5 bg-[#008f89] text-white rounded font-semibold hover:bg-[#007a75] shadow-sm">
              Nhập chi phí | <ChevronDown className="w-4 h-4" />
            </button>
      ...
      <div className="p-6">
        <div className="bg-white rounded border border-slate-300 shadow-sm overflow-hidden">
          <table className="w-full text-sm text-left border-collapse select-none">
            <thead className="bg-[#bce6e4] text-[#006b66]">
              <tr>
                ...
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Mã khoản chi phí</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Tên khoản chi phí</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold text-right">Tổng giá trị</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold text-right">Giá trị đã phân bổ</th>
              ...
              {loading ? (
                ...
                prepaids.map((item, idx) => (
                  <tr key={idx} className="border-b border-slate-200 hover:bg-emerald-50/30">
                    ...
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700 font-semibold">{item.maKhoanChiPhi}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700">{item.tenKhoanChiPhi}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-right font-semibold">{formatNum(Number(item.tongGiaTri))}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-right font-semibold">{formatNum(Number(item.giaTriDaPhanBo))}</td>
            ...
            <tfoot className="bg-[#f0f0f0]">
              <tr>
                ...
                <td className="px-3 py-2 font-bold text-right border-r border-slate-300 text-emerald-700">{formatNum(mainTotalGT)}</td>
                <td className="px-3 py-2 font-bold text-right border-r border-slate-300 text-emerald-700">{formatNum(mainTotalPB)}</td>
          ...
          <div className="p-3 border-t border-slate-200 text-sm flex justify-between items-center text-slate-500">
            <div>Tổng số: <span className="font-bold text-slate-700">{prepaids.length}</span> bản ghi</div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-[#f4f7f6] flex flex-col h-screen animate-in fade-in">
          <div className="px-6 py-3 flex items-center justify-between border-b border-slate-200 bg-white shadow-sm">
            <h1 className="text-xl font-bold text-slate-800">Nhập chi phí trả trước</h1>
              ...
              <div className="flex-1 overflow-y-auto relative">
                <table className="w-full text-sm text-left border-collapse select-none">
                  <thead className="bg-[#bce6e4] text-[#006b66] sticky top-0 z-10 border-b border-slate-300 shadow-sm">
                    <tr>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-48">Mã khoản chi phí</th>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-64">Tên khoản chi phí</th>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-48 text-right">Tổng giá trị</th>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-48 text-right">Giá trị đã phân bổ</th>
                    ...
                    {gridItems.map((item) => (
                      <tr key={item.id} className="border-b border-slate-200 hover:bg-emerald-50/30">
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <GenericCombobox 
                            value={item.maKhoanChiPhi} 
                            items={MOCK_PREPAID}
                            codeLabel="Mã CP"
                            nameLabel="Tên chi phí"
                            onChange={(code) => handleCellChange(item.id, 'maKhoanChiPhi', code)} 
                            onNameChange={(name) => handleCellChange(item.id, 'tenKhoanChiPhi', name)}
                          />
                        </td>
                        <td className="px-4 py-2 border-r border-slate-200 text-slate-700">{item.tenKhoanChiPhi}</td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <NumberInput value={item.tongGiaTri} onChange={(val) => handleCellChange(item.id, 'tongGiaTri', val)} />
                        </td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <NumberInput value={item.giaTriDaPhanBo} onChange={(val) => handleCellChange(item.id, 'giaTriDaPhanBo', val)} />
                        </td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                  ...
                  <tfoot className="bg-[#f0f0f0] text-slate-800 sticky bottom-0 z-10 border-t border-slate-300 shadow-[0_-2px_4px_rgba(0,0,0,0.05)]">
                    <tr>
                      ...
                      <td className="px-4 py-2 font-bold text-right border-r border-slate-300 text-lg text-emerald-700">{formatNum(gridTotalGT)}</td>
                      <td className="px-4 py-2 font-bold text-right border-r border-slate-300 text-lg text-emerald-700">{formatNum(gridTotalPB)}</td>
