...
const MOCK_EMPLOYEES = [
  ...
  { code: "NV004", name: "Phạm Thị D" },
  { code: "NV005", name: "Hoàng Văn E" }
];

export default function OBEmployeeDebtPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await fetch("http://localhost:3001/api/opening-balances/employee-debt?namKeToan=2026");
  ...
  const openModal = () => {
    if (employees.length > 0) {
      setGridItems(employees.map((c, idx) => ({ ...c, id: idx.toString() })));
    } else {
      handleAddRow();
    }
    setIsModalOpen(true);
    ...
    try {
      const payload = {
        ...
        items: gridItems.map(item => ({
          ...
          soTienNhanVienUng: item.soTienNhanVienUng || 0,
          soTienPhaiTra: item.soTienPhaiTra || 0,
          employeeId: Date.now() + Math.floor(Math.random() * 1000)
        }))
      };

      const res = await fetch("http://localhost:3001/api/opening-balances/employee-debt/batch", {
        method: "POST",
      ...
      if (res.ok) {
        alert("Lưu công nợ nhân viên thành công!");
        fetchData();
  ...
  const mainTotalDebit = employees.reduce((sum, item) => sum + Number(item.soTienNhanVienUng || 0), 0);
  const mainTotalCredit = employees.reduce((sum, item) => sum + Number(item.soTienPhaiTra || 0), 0);
  
  const gridTotalDebit = gridItems.reduce((sum, item) => sum + Number(item.soTienNhanVienUng || 0), 0);
    ...
    <div className="bg-[#f4f7f6] min-h-[calc(100vh-4rem)] pb-32">
      ...
      <div className="px-6 py-4 flex flex-col gap-4 bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">Công nợ nhân viên</h1>
      ...
      <div className="p-6">
        <div className="bg-white rounded border border-slate-300 shadow-sm overflow-hidden">
          <table className="w-full text-sm text-left border-collapse select-none">
            <thead className="bg-[#bce6e4] text-[#006b66]">
              <tr>
                ...
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Mã nhân viên</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Tên nhân viên</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Phòng ban</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold text-right">Dư Nợ (NV ứng)</th>
              ...
              {loading ? (
                ...
                employees.map((item, idx) => (
                  <tr key={idx} className="border-b border-slate-200 hover:bg-emerald-50/30">
                    ...
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700 font-semibold">{item.maNhanVien}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700">{item.tenNhanVien}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700">{item.phongBan}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-right font-semibold">{formatNum(Number(item.soTienNhanVienUng))}</td>
          ...
          <div className="p-3 border-t border-slate-200 text-sm flex justify-between items-center text-slate-500">
            <div>Tổng số: <span className="font-bold text-slate-700">{employees.length}</span> bản ghi</div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-[#f4f7f6] flex flex-col h-screen animate-in fade-in">
          <div className="px-6 py-3 flex items-center justify-between border-b border-slate-200 bg-white shadow-sm">
            <h1 className="text-xl font-bold text-slate-800">Nhập công nợ nhân viên</h1>
              ...
              <div className="flex-1 overflow-y-auto relative">
                <table className="w-full text-sm text-left border-collapse select-none">
                  <thead className="bg-[#bce6e4] text-[#006b66] sticky top-0 z-10 border-b border-slate-300 shadow-sm">
                    <tr>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-48">Mã nhân viên</th>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-64">Tên nhân viên</th>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9]">Phòng ban</th>
                      ...
                      <tr key={item.id} className="border-b border-slate-200 hover:bg-emerald-50/30">
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <GenericCombobox 
                            ...
                            onChange={(code) => handleCellChange(item.id, 'maNhanVien', code)} 
                            onNameChange={(name) => handleCellChange(item.id, 'tenNhanVien', name)}
                          />
                        </td>
                        <td className="px-4 py-2 border-r border-slate-200 text-slate-700">{item.tenNhanVien}</td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <TextInput value={item.phongBan} onChange={(val) => handleCellChange(item.id, 'phongBan', val)} placeholder="Phòng ban" />
                        </td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <NumberInput value={item.soTienNhanVienUng} onChange={(val) => handleCellChange(item.id, 'soTienNhanVienUng', val)} />
                        </td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
