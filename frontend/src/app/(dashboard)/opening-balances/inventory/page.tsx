...
const MOCK_VTHH = [
  { code: "VTHH001", name: "Xi măng Hà Tiên 1 PC40" },
  { code: "VTHH002", name: "Thép cuộn CB240" },
  { code: "VTHH003", name: "Gạch Tuynel 8x8x18" },
  { code: "VTHH004", name: "Cát xây tô" },
  { code: "VTHH005", name: "Đá 1x2" }
];

export default function OBInventoryPage() {
  ...
  const fetchData = async () => {
    try {
      setLoading(true);
      const res = await fetch("http://localhost:3001/api/opening-balances/inventory?namKeToan=2026");
  ...
  const openModal = () => {
    if (inventory.length > 0) {
      setGridItems(inventory.map((c, idx) => ({ ...c, id: idx.toString() })));
    } else {
      handleAddRow();
    }
    setIsModalOpen(true);
  ...
  const handleAddRow = () => {
    setGridItems(prev => [
      ...prev,
      {
        ...
        maVthh: "VTHH001",
        tenVthh: "Xi măng Hà Tiên 1 PC40",
    ...
    try {
      const payload = {
        ...
        items: gridItems.map(item => ({
          maVthh: item.maVthh,
          tenVthh: item.tenVthh,
          kho: item.kho,
          soLuong: item.soLuong || 0,
          donGia: item.donGia || 0,
          giaTri: item.giaTri || 0,
          inventoryId: Date.now() + Math.floor(Math.random() * 1000)
        }))
      };

      const res = await fetch("http://localhost:3001/api/opening-balances/inventory/batch", {
        method: "POST",
  ...
  const mainTotalValue = inventory.reduce((sum, item) => sum + Number(item.giaTri || 0), 0);
    ...
    <div className="bg-[#f4f7f6] min-h-[calc(100vh-4rem)] pb-32">
      ...
      <div className="px-6 py-4 flex flex-col gap-4 bg-white border-b border-slate-200 sticky top-0 z-30 shadow-sm">
        <div className="flex items-center justify-between">
          <h1 className="text-2xl font-bold text-slate-800 flex items-center gap-2">Tồn kho vật tư, hàng hóa</h1>
        ...
        <div className="flex items-center justify-between text-sm">
          ...
          <div className="flex items-center gap-3">
            ...
            <button onClick={openModal} className="flex items-center gap-2 px-6 py-1.5 bg-[#008f89] text-white rounded font-semibold hover:bg-[#007a75] shadow-sm">
              Nhập tồn kho | <ChevronDown className="w-4 h-4" />
            </button>
      ...
      <div className="p-6">
        <div className="bg-white rounded border border-slate-300 shadow-sm overflow-hidden">
          <table className="w-full text-sm text-left border-collapse select-none">
            <thead className="bg-[#bce6e4] text-[#006b66]">
              <tr>
                ...
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Mã VTHH</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Tên VTHH</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold">Kho</th>
                <th className="px-3 py-2 border-r border-[#a8dbd9] font-bold text-right">Số lượng</th>
                <th className="px-3 py-2 font-bold text-right">Giá trị</th>
              </tr>
            </thead>
            <tbody>
              {loading ? (
                ...
                inventory.map((item, idx) => (
                  <tr key={idx} className="border-b border-slate-200 hover:bg-emerald-50/30">
                    ...
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700 font-semibold">{item.maVthh}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700">{item.tenVthh}</td>
                    <td className="px-3 py-2 border-r border-slate-200 text-slate-700">{item.kho}</td>
          ...
          <div className="p-3 border-t border-slate-200 text-sm flex justify-between items-center text-slate-500">
            <div>Tổng số: <span className="font-bold text-slate-700">{inventory.length}</span> bản ghi</div>
      ...
      {isModalOpen && (
        <div className="fixed inset-0 z-50 bg-[#f4f7f6] flex flex-col h-screen animate-in fade-in">
          <div className="px-6 py-3 flex items-center justify-between border-b border-slate-200 bg-white shadow-sm">
            <h1 className="text-xl font-bold text-slate-800">Nhập tồn kho vật tư, hàng hóa</h1>
              ...
              <div className="flex-1 overflow-y-auto relative">
                <table className="w-full text-sm text-left border-collapse select-none">
                  <thead className="bg-[#bce6e4] text-[#006b66] sticky top-0 z-10 border-b border-slate-300 shadow-sm">
                    <tr>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-48">Mã VTHH</th>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-64">Tên VTHH</th>
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-48">Kho</th>
                      ...
                      <th className="px-4 py-2 font-bold border-r border-[#a8dbd9] w-48 text-right">Giá trị</th>
                      ...
                      <tr key={item.id} className="border-b border-slate-200 hover:bg-emerald-50/30">
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <GenericCombobox 
                            ...
                            codeLabel="Mã VTHH"
                            nameLabel="Tên VTHH"
                            onChange={(code) => handleCellChange(item.id, 'maVthh', code)} 
                            onNameChange={(name) => handleCellChange(item.id, 'tenVthh', name)}
                          />
                        </td>
                        <td className="px-4 py-2 border-r border-slate-200 text-slate-700">{item.tenVthh}</td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
                          <TextInput value={item.kho} onChange={(val) => handleCellChange(item.id, 'kho', val)} placeholder="Tên kho" />
                        </td>
                        <td className="px-2 py-1 border-r border-slate-200 bg-white">
