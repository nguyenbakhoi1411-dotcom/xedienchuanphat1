...
import { useInventoryIssues } from "@/features/inventory/hooks";
...
export default function InventoryOutboundPage() {
  ...
  const { data: issuesData, isLoading } = useInventoryIssues({
  ...
  const filteredItems = useMemo(() => {
    ...
    return issuesData.items.filter(item => {
      const q = searchQuery.trim().toLowerCase();
      if (!q) return true;
      return (
        (item.issueNo && item.issueNo.toLowerCase().includes(q)) ||
      ...
      <div className="bg-white px-6 py-4 border-b border-gray-200 flex items-center justify-between">
        <div className="flex items-center gap-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-0.5">Quản lý các giao dịch xuất hàng khỏi kho</p>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-gray-100 flex flex-col h-full">
            ...
            <div className="flex items-center gap-3 flex-1 min-w-[300px]">
              <div className="relative flex-1 max-w-md">
                ...
                <input
                  ...
                  className="w-full h-10 pl-10 pr-4 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/50 bg-gray-50 hover:bg-white transition-colors"
                />
              </div>
              ...
              <select
                ...
                className="h-10 px-4 rounded-lg border border-gray-200 text-sm focus:outline-none focus:ring-2 focus:ring-orange-500/50 bg-white min-w-[160px]"
              >
                ...
                <tbody className="divide-y divide-gray-50">
                  {filteredItems.map((item: any) => (
                    <tr key={item.id} className="hover:bg-orange-50/30 transition-colors group">
                      ...
                      <td className="py-3.5 px-4 font-mono text-orange-700 font-medium">
                        {item.issueNo}
                      </td>
                      ...
                      <td className="py-3.5 px-4 text-right">
                        <Button
                          variant="outline"
                          size="sm"
                          className="text-gray-500 hover:text-orange-700 border-gray-200 bg-white"
                        >
          ...
          {issuesData && issuesData.totalPages > 1 && (
             <div className="p-4 border-t border-gray-100 flex items-center justify-between text-sm text-gray-500">
               <span>Hiển thị trang {page + 1} / {issuesData.totalPages}</span>
               <div className="flex items-center gap-2">
                 ...
                 <Button 
                   variant="outline" 
                   size="sm"
                   disabled={page >= issuesData.totalPages - 1}
                   onClick={() => setPage(p => p + 1)}
                 >
                   Sau
                 </Button>
