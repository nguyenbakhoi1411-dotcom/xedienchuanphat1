...
const bankPaymentSchema = z.object({
  ...
  nganHangDoiUng: z.string().nullable().optional(),
  soTkDoiUng: z.string().nullable().optional(),
  tenChuTkDoiUng: z.string().nullable().optional(),
});
...
export function BankPaymentForm({ accountId, onSuccess, onError, onCancel }: BankPaymentFormProps) {
  const { data: bankAccounts = [] } = useBankAccounts();
  const { mutate: createPayment, isPending } = useCreateBankPayment();

  const [paymentType, setPaymentType] = useState('9. Chi khác');
  const [showPaymentTypeDropdown, setShowPaymentTypeDropdown] = useState(false);
  const [items, setItems] = useState<any[]>([{ id: Date.now(), dienGiai: 'Chi tiền cho', tkNo: '', tkCo: '11211', soTien: 0, doiTuongId: '', tenDoiTuong: '', tenDonVi: '' }]);
  const [showAccounts, setShowAccounts] = useState(false);

  const { control, handleSubmit, watch, setValue } = useForm<BankPaymentFormData>({
  ...
  useEffect(() => {
    if (tenDoiTuong && lyDo === 'Chi tiền cho') {
      setValue('lyDo', `Chi tiền cho ${tenDoiTuong}`);
    }
  }, [tenDoiTuong, lyDo, setValue]);

  const addItem = () => {
    setItems([...items, { id: Date.now(), dienGiai: watch('lyDo') || '', tkNo: '', tkCo: '11211', soTien: 0, doiTuongId: '', tenDoiTuong: '', tenDonVi: '' }]);
  };

  const removeAllItems = () => {
    setItems([{ id: Date.now(), dienGiai: watch('lyDo') || '', tkNo: '', tkCo: '11211', soTien: 0, doiTuongId: '', tenDoiTuong: '', tenDonVi: '' }]);
  };
  ...
  const onSubmit = (data: BankPaymentFormData) => {
    const validItems = items.filter(item => item.tkNo && item.soTien > 0);
    if (validItems.length === 0) {
      toast.error("Vui lòng nhập ít nhất một dòng hạch toán hợp lệ (có TK Nợ và Số tiền > 0)");
      return;
    }
    ...
    createPayment(payload, {
      onSuccess: () => {
        toast.success("Tạo ủy nhiệm chi thành công!");
        onSuccess?.();
      },
      onError: (error: any) => {
        const message = error.response?.data?.message || 'Lỗi tạo ủy nhiệm chi';
        toast.error(message);
        onError?.(message);
      },
    });
  ...
  return (
    <div className="flex flex-col h-full bg-[#f4f5f8] text-[13px] text-gray-800">
      ...
      <div className="flex items-center justify-between px-4 py-2 border-b border-gray-200 bg-white">
        <div className="flex items-center gap-3">
          ...
          <div className="relative">
            ...
            >
              {paymentType} <ChevronDown className="h-4 w-4" />
            </button>
            {showPaymentTypeDropdown && (
              <div className="absolute top-full left-0 mt-1 w-72 bg-white border border-gray-200 shadow-lg rounded-sm z-50">
                <ul className="py-1">
                  {['1. Trả tiền nhà cung cấp (không theo hóa đơn)', '2. Trả các khoản vay', '3. Tạm ứng cho nhân viên', '4. Chi mua ngoài có hóa đơn', '5. Trả lương tạm ứng cho nhân viên', '9. Chi khác'].map(t => (
                    <li 
                      key={t}
                      className="px-4 py-1.5 hover:bg-gray-100 cursor-pointer"
                      onClick={() => { setPaymentType(t); setShowPaymentTypeDropdown(false); }}
                    >
                      {t}
                    </li>
                  ))}
      ...
      <div className="flex-1 overflow-auto">
        <form id="bank-payment-form" onSubmit={handleSubmit(onSubmit)} className="p-4 space-y-4">
            ...
            <div className="flex gap-8">
              ...
              <div className="flex-1 space-y-2.5">
                <div className="flex items-center">
                  <label className="w-32 font-medium shrink-0">Tài khoản chi <span className="text-red-500">*</span></label>
                ...
                <div className="flex items-center">
                  <label className="w-32 font-medium shrink-0">Tài khoản nhận</label>
                  <div className="flex-1 relative">
                    <Controller
                      name="soTkDoiUng"
                      control={control}
                ...
                <div className="flex items-center">
                  <label className="w-32 font-medium shrink-0">Nhân viên</label>
                  <div className="flex-1 relative">
                    <input
                      type="text"
                      className="w-full h-7 px-2 border border-gray-300 rounded-sm focus:border-blue-500 focus:outline-none"
                ...
                <div className="flex items-start pt-1">
                  <label className="w-32 font-medium shrink-0">Nội dung TT</label>
                  <Controller
                    name="lyDo"
                    control={control}
                    render={({ field }) => (
