...
import { CashPaymentDto } from "@/features/cash/types";
...
type Props = {
  ...
  onPrint: (item: CashPaymentDto) => void;
};

export function PaymentTable({ data, loading, onConfirm, onCancel, onPrint }: Props) {
  if (loading) {
    return (
  ...
  const getTypeLabel = (type: string) => {
    switch (type) {
      case "PURCHASE": return "Chi mua hàng";
      case "SALARY": return "Chi lương";
      case "OPERATING": return "Chi phí vận hành";
      case "REFUND": return "Chi hoàn trả";
        ...
        <table className="w-full text-sm text-left">
          <thead className="bg-slate-50 text-slate-600 border-b">
            <tr>
              ...
              <th className="px-4 py-3 font-medium">Mã PC</th>
          ...
          <tbody className="divide-y divide-slate-100">
            {data.map((item) => (
              <tr key={item.id} className="hover:bg-slate-50 transition-colors group">
                <td className="px-4 py-3">{new Date(item.paymentDate).toLocaleDateString("vi-VN")}</td>
                <td className="px-4 py-3 font-medium text-slate-800">{item.voucherNo}</td>
                <td className="px-4 py-3">{getTypeLabel(item.paymentType)}</td>
                <td className="px-4 py-3 text-slate-600">
                  <div className="max-w-[150px] truncate" title={item.payeeName}>
                    {item.payeeName || "—"}
                  </div>
                </td>
                    ...
                    {item.status === "DRAFT" && (
                      <>
                        <Button variant="ghost" size="icon" onClick={() => {
                          if (window.confirm("Xác nhận ghi sổ phiếu chi này?")) {
                            onConfirm(item.id);
                          }
                        ...
                        <Button variant="ghost" size="icon" onClick={() => {
                          if (window.confirm("Hủy bỏ phiếu chi này?")) {
                            onCancel(item.id);
                          }
                        }} title="Hủy phiếu">
