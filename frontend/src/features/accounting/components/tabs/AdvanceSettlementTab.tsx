...
import { Save, Search, Settings, X, Plus, Paperclip } from "lucide-react";
import { EditableDataTable, Column } from "@/components/ui/EditableDataTable";

export function AdvanceSettlementTab() {
  const [items, setItems] = useState<any[]>([{
    id: "1", description: "Quyết toán tạm ứng", debitAccount: "141", creditAccount: "", amount: 0, debitObject: "", debitObjectName: "", creditObject: "", creditObjectName: ""
  }]);

  const columns: Column<any>[] = [
    { key: "description", header: "Diễn giải", type: "text", width: "250px" },
    ...
    <div className="flex flex-col h-full bg-slate-50">
      <div className="bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
            ...
            Chứng từ quyết toán tạm ứng QTTU00001
          </h2>
          <label className="flex items-center gap-2 text-sm text-slate-600 ml-4"><input type="checkbox" className="text-emerald-600" /> Quyết toán cho từng lần tạm ứng</label>
        </div>
      ...
      <div className="flex-1 overflow-auto p-4">
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 mb-4 flex gap-6">
          <div className="flex-1 space-y-4">
            <div className="grid grid-cols-12 gap-4 items-center">
              <label className="col-span-2 text-sm font-semibold text-slate-700">Nhân viên</label>
              <div className="col-span-10">
                <select className="w-64 px-3 py-1.5 border border-slate-300 rounded-md text-sm outline-none focus:border-emerald-500">
                  <option></option>
                </select>
              </div>
            </div>
            <div className="grid grid-cols-12 gap-4 items-center">
              ...
              <div className="col-span-10">
                <input type="text" className="w-full px-3 py-1.5 border border-slate-300 rounded-md text-sm outline-none focus:border-emerald-500" defaultValue="Quyết toán tạm ứng" />
              </div>
            </div>
            <div className="grid grid-cols-12 gap-4 items-center">
          ...
          <div className="w-[300px] border-l border-slate-100 pl-6 space-y-3">
            ...
            <div className="flex justify-between items-center">
              ...
              <input type="text" className="w-36 px-2 py-1 border border-slate-300 rounded text-sm outline-none text-right font-semibold" defaultValue="QTTU00001" />
            </div>
          </div>
        </div>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col min-h-[400px]">
          <div className="flex items-center justify-between border-b border-slate-200 px-4 bg-slate-50">
            ...
            <div className="flex items-center gap-4">
              <label className="flex items-center gap-2 text-xs text-slate-600"><input type="checkbox" className="text-emerald-600" /> Hạch toán gộp nhiều hóa đơn</label>
      ...
      <div className="bg-slate-800 text-white px-4 py-3 flex justify-between items-center z-20 sticky bottom-0">
        ...
        <div className="flex items-center gap-2">
          ...
          <button className="px-4 py-1.5 text-sm font-medium bg-[#008f89] rounded hover:bg-[#007a75] flex items-center gap-1">Cất và Thêm ▾</button>
        </div>
      </div>
    </div>
  );
