...
export function SalesContractTab() {
  const [items, setItems] = useState<ContractItem[]>([{
    id: "1",
    productCode: "SP002",
    productName: "Dịch vụ bảo trì 1 năm",
    unit: "Gói",
    requiredQuantity: 1,
    deliveredQuantity: 0,
    unitPrice: 5000000,
  ...
  const columns: Column<ContractItem>[] = [
    ...
    { key: "requiredQuantity", header: "Số lượng yêu cầu", type: "number", width: "140px" },
    { key: "deliveredQuantity", header: "Số lượng đã giao", type: "number", width: "140px", readOnly: true },
  ...
  const handleItemsChange = (newItems: ContractItem[]) => {
    const calculated = newItems.map(item => {
      const amount = (item.requiredQuantity || 0) * (item.unitPrice || 0);
      ...
      <div className="bg-white border-b border-slate-200 px-4 py-3 flex items-center justify-between sticky top-0 z-10 shadow-sm">
        <div className="flex items-center gap-4">
          <h2 className="text-lg font-bold text-slate-800 flex items-center gap-2">
            Hợp đồng bán HĐB00001
          </h2>
          <div className="relative">
            <Search className="w-4 h-4 absolute left-3 top-1/2 -translate-y-1/2 text-slate-400" />
            <input type="text" placeholder="Nhập số đơn đặt hàng..." className="pl-9 pr-4 py-1.5 bg-slate-100 border-none rounded-full text-sm w-64 focus:ring-2 focus:ring-emerald-500 outline-none" />
          </div>
        </div>
        <div className="flex items-center gap-2">
          <button className="px-4 py-1.5 text-sm font-medium text-white bg-emerald-600 rounded-lg hover:bg-emerald-700 flex items-center gap-2 shadow-sm">
            <Save className="w-4 h-4" /> Cất và Thêm
          </button>
        </div>
      </div>
      ...
      <div className="flex-1 overflow-auto p-4">
          ...
          <div className="grid grid-cols-1 md:grid-cols-4 gap-6">
            <div className="col-span-3 grid grid-cols-2 gap-4">
              ...
              <div className="grid grid-cols-2 gap-4 col-span-2">
                <div>
                  <label className="block text-xs font-semibold text-slate-500 mb-1">Hạn thanh toán</label>
                  <input type="date" className="w-full px-3 py-2 border border-slate-300 rounded-md text-sm" />
                </div>
                <div className="flex items-center gap-2 pt-5">
                  <input type="checkbox" id="auto_liquidate" className="rounded text-emerald-600 focus:ring-emerald-500" defaultChecked />
                  <label htmlFor="auto_liquidate" className="text-sm text-slate-700 font-medium">Tự động chuyển Đã thanh lý khi đủ điều kiện</label>
                </div>
              </div>
            </div>
            ...
            <div className="bg-slate-50 p-4 rounded-lg border border-slate-100 flex flex-col justify-between">
              <div className="space-y-3">
                <div className="flex justify-between items-center border-b border-slate-200 pb-2">
                  <span className="text-xs font-semibold text-slate-500">Số hợp đồng</span>
                  <input type="text" className="text-sm bg-transparent font-bold text-slate-800 outline-none text-right w-24" defaultValue="HĐB00001" />
                </div>
                <div className="flex justify-between items-center border-b border-slate-200 pb-2">
                  <span className="text-xs font-semibold text-slate-500">Ngày ký</span>
                ...
                <div className="flex justify-between items-center border-b border-slate-200 pb-2">
                  <span className="text-xs font-semibold text-slate-500">Tình trạng hợp đồng</span>
                  <select className="text-sm bg-transparent font-medium text-amber-600 outline-none text-right appearance-none">
                    <option>Chưa thực hiện</option>
                  </select>
                </div>
              </div>
              <div className="mt-4 text-right">
                <p className="text-xs font-bold text-slate-400 uppercase tracking-wider mb-1">Giá trị hợp đồng</p>
        ...
        <div className="bg-white rounded-xl shadow-sm border border-slate-200 overflow-hidden flex flex-col min-h-[400px]">
          <div className="flex gap-1 bg-slate-50 border-b border-slate-200 px-2 pt-2">
            <button className="px-4 py-2 text-sm font-semibold text-emerald-700 bg-white border-t-2 border-emerald-500 rounded-t-lg">Danh sách hàng hóa dịch vụ</button>
            <button className="px-4 py-2 text-sm font-medium text-slate-500 hover:text-slate-700">Điều khoản thanh toán</button>
          </div>
